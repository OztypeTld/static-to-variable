...
def test_chunked_carrier_names_are_recognized_for_authored_owners_only() -> None:
    ...
    assert preserved == frozenset({"curve", "curve.stv-semantic16x", "curve.stv-semantic64x"})
    assert _exact_authored_carriers(font, authored, preserved) == frozenset({LOSSY, EXACT})
